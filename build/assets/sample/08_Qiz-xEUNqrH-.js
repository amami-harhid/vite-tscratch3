import { _ as w, a as h, __tla as __tla_0 } from "../index-BP3kCZCx.js";
Promise.all([
    (()=>{
        try {
            return __tla_0;
...
`, t = {
    ...
    class c {
        ...
        static textFilter(n) {
            const s = [];
            for (const l of n)for (const e of l){
                const u = e.replaceAll(/<br\/>/g, "").replaceAll(/\s/g, "").split("");
                for (const g of u)g != " " && (s.includes(g) || s.push(g));
            }
            return s.sort().join("");
        }
        static get fontFaceUrlJPGuidance() {
            const n = "UoqMunThenKhung", s = c.textsGuidance, r = c.textFilter(s);
            return `https://fonts.googleapis.com/css2?family=${n}&display=swap&text='+${encodeURIComponent(r)}`;
        }
        static get fontFaceUrlJP() {
            const n = "UoqMunThenKhung", s = c.textsQizJp, r = c.textFilter(s);
            return `https://fonts.googleapis.com/css2?family=${n}&display=swap&text='+${encodeURIComponent(r)}`;
        }
        static get fontFaceUrlEN() {
            const n = "UoqMunThenKhung", s = c.textsAnswserJp, r = c.textFilter(s);
            return `https://fonts.googleapis.com/css2?family=${n}&display=swap&text='+${encodeURIComponent(r)}`;
        }
    }
    const L = "/assets/blackCat01-5DfUgPqk.svg", F = "/assets/blackCat02-BDtzF7Oy.svg", C = "/assets/blackCat03-BisdsYxq.svg", G = "/assets/den-JIQ9x_Nd.mp3", P = "/assets/OK-Jdg1c7Sk.mp3", A = "/assets/NG-BYQBgWQH.mp3", I = c.textsGuidance, N = c.textsQizJp, v = c.textsAnswserJp, E = "Sprite";
    let x, a, f, d, b, S;
    w.preload = async function() {
        this.Image.load(L, t.QUESTION), this.Image.load(F, t.ZANNEN), this.Image.load(C, t.SEIKAI), this.Font.load(c.fontFaceUrlJPGuidance, t.GoogleFontsJPGuidance), this.Font.load(c.fontFaceUrlJP, t.GoogleFontsJP), this.Font.load(c.fontFaceUrlEN, t.GoogleFontsEN), this.Sound.load(G, t.DEN), this.Sound.load(P, t.OK), this.Sound.load(A, t.NG);
    };
    w.prepare = async function() {
        x = new h.Stage, x.SvgText.add("BackDrop", T), S = new h.Sprite("reference"), S.Font.add(t.GoogleFontsJP), S.Motion.Position.xy = [
            180,
            -160
        ], S.Looks.Size.scale = [
            50,
        ...
        a = new h.Sprite("quizeCat"), a.Image.add(t.QUESTION), a.Image.add(t.ZANNEN), a.Image.add(t.SEIKAI), a.Motion.Position.xy = [
            0,
            -100
        ], a.Looks.Size.scale = [
            60,
            50
        ], a.Sound.add(t.DEN), a.Sound.add(t.OK), a.Sound.add(t.NG), f = new h.Sprite(E), f.Font.add(t.GoogleFontsJPGuidance), f.Motion.Position.xy = [
            0,
            80
        ], f.Looks.Size.scale = [
            100,
        ...
        for (const e of h.Iterator(I.length)){
            const i = I[e];
            f.SvgText.addTexts(`text01-${e}`, i, l);
        }
        d = new h.Sprite(E), d.Motion.Position.xy = [
            0,
            50
        ...
        for (const e of h.Iterator(N.length)){
            const i = N[e], u = i[0];
            i.shift();
            const k = {
            ...
            d.SvgText.addTexts(`text02-${e}-${u}`, i, k);
        }
        b = new h.Sprite(E), b.Font.add(t.GoogleFontsEN), b.Motion.Position.xy = [
            0,
            80
        ], b.Looks.Size.scale = [
            80,
        ...
        for (const e of h.Iterator(v.length)){
            const i = v[e], y = {
            ...
            b.SvgText.addTexts(`text03-${e}`, i, y);
        }
    };
    w.setting = async function() {
        let n = 0;
        x.Event.whenBroadcastReceived(o.QUESTION, async function(s, r, l) {
            const e = await this.Sensing.askAndWait("番号を入れてね");
            e == "1" || e == "2" || e == "3" ? (this.Event.broadcast(o.ANSWER, s), e == r ? (n += 1, this.Event.broadcast(o.SEIKAI)) : this.Event.broadcast(o.ZANNEN), await this.Control.waitUntil(()=>this.Sensing.isKeyDown(h.Keyboard.SPACE)), s + 1 > l.length - 1 ? (console.log("OWARI"), this.Event.broadcast(o.COMPLETE)) : this.Event.broadcast(o.QIZ, s + 1)) : this.Event.broadcast(o.QIZ, s);
        }), a.Event.whenFlag(async function() {
            this.Looks.hide(), n = 0, this.Event.broadcast(o.INIT);
            ...
            n > 5 ? this.Looks.Costume.name = t.SEIKAI : this.Looks.Costume.name = t.ZANNEN;
            ...
            this.Looks.hide(), this.Motion.Position.xy = [
                ...
                100,
                100
            ], this.Looks.Costume.name = "text01-1", this.Looks.show(), await this.Control.waitUntil(()=>this.Sensing.isKeyDown(h.Keyboard.SPACE)), this.Event.broadcast(o.QIZ, 0);
            ...
            const i = d.Looks.Costume.names.length;
            n == i ? this.SvgText.replaceTexts("text01-tokuten", [
                "満点、素晴らしい！"
            ], e) : n / i > .7 ? this.SvgText.replaceTexts("text01-tokuten", [
                `${i}問中、正解(${n})、よく出来ました`
            ], e) : n / i > .3 ? this.SvgText.replaceTexts("text01-tokuten", [
                `${i}問中、正解(${n})、まあまあだね`
            ], e) : this.SvgText.replaceTexts("text01-tokuten", [
                `${i}問中、正解(${n})、もっと頑張ろう`
            ], e);
        }), d.Event.whenBroadcastReceived(o.QIZ, async function(s) {
            const r = this.Looks.Costume.names, l = r[s];
            this.Looks.Costume.name = l;
            const e = l.replace(/text02-\d+-/, "");
            this.Motion.Position.xy = [
                0,
                80
            ], this.Looks.show(), await this.Control.wait(.5), await this.Event.broadcastAndWait(o.QUESTION, s, e, r);
            ...
            this.Looks.hide();
        }), b.Event.whenBroadcastReceived(o.ANSWER, async function(s) {
            this.Motion.Position.xy = [
                0,
                80
            ], this.Looks.Size.scale = [
                80,
                80
            ], this.Looks.Costume.name = `text03-${s}`, this.Looks.show();
        });
    };
});
