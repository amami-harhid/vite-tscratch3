import { a as h, c as t, __tla as __tla_0 } from "../index-D2v2F40U.js";
Promise.all([
    (()=>{
        try {
            return __tla_0;
    ...
    const o = {
        ...
        Debris03: "Debris03",
        Bullet: "bullet"
    }, w = 30, m = 30, g = 15, k = function(c) {
        return `
<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${m}" viewBox="0 0 ${w} ${m}">
    <circle cx="${g}" cy="${g}" r="${g}" fill="${c}"/>
</svg>
`;
    }, y = `
...
`;
    ...
    h.setting = async function() {
        r.Event.whenFlag(async function*() {
            ...
            this.Looks.Costume.name = o.Bullet, this.Motion.Move.toSprite(s), this.Motion.Direction.degree = 0;
            const e = 10;
            for(this.Looks.show(); !this.Sensing.isTouchingEdge();)this.Motion.Move.steps(e), yield;
            this.Control.remove();
            ...
            for(;;)this.Motion.Position.x = t.randomInteger(-240, 240), await this.Control.wait(1), this.Control.clone(), this.Looks.Costume.name = e[t.randomInteger(0, e.length - 1)], yield;
        }), i.Control.whenCloned(async function*() {
            const e = t.randomInteger(50, 100);
            this.Looks.Size.scale = [
                e,
                e
            ];
            const d = t.getRandomValueInRange(-1, 5);
            for(this.Looks.show();;){
                ...
                this.Motion.Position.y += t.randomInteger(-5, 1) / 2, this.Motion.Direction.degree += d, yield;
            }
